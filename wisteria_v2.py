...
"""
Wisteria Research Hypothesis Generator v2.0 - Interactive Mode

Usage:
    python wisteria_v2.py <research_goal_file.txt> --model <model_shortname> [--output <output_file.json>]
    python wisteria_v2.py --goal "<research_goal_text>" --model <model_shortname> [--output <output_file.json>]
...
Examples:
    python wisteria_v2.py research_goal.txt --model gpt41
    python wisteria_v2.py --goal "How can we improve renewable energy storage efficiency?" --model scout
    python wisteria_v2.py --goal "What causes neurodegenerative diseases?" --model gpt41 --output my_hypotheses.json
...
4) Ensures each new hypothesis is different from previous ones
5) Outputs all hypotheses and refinements to JSON file
"""
...
def display_hypotheses(hypotheses):
    ...
    for i, hypothesis in enumerate(hypotheses, 1):
        ...
        version = hypothesis.get("version", f"{i}.0")
...
def get_user_feedback():
    ...
    Returns:
        str: User feedback text, or None if user wants to quit or generate new hypothesis
    """
    print("\n" + "-" * 60)
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Wisteria Research Hypothesis Generator v2.0 - Interactive Mode')
...
def interactive_hypothesis_session(research_goal, model_config):
    """
    ...
    try:
        ...
        version_tracker[hypothesis_counter] = 0  # Start at version x.0
        first_hypothesis["hypothesis_number"] = hypothesis_counter
        first_hypothesis["version"] = f"{hypothesis_counter}.0"
    ...
    while True:
        ...
        if feedback == "QUIT":
            ...
            try:
                ...
                new_hypothesis["version"] = f"{hypothesis_counter}.0"
            ...
            try:
                ...
                improved_hypothesis["version"] = f"{hypothesis_number}.{version_tracker[hypothesis_number]}"
...
def main():
    ...
    print(f"Wisteria Research Hypothesis Generator v2.0 - Interactive Mode")
    ...
    try:
        all_hypotheses = interactive_hypothesis_session(research_goal, model_config)
